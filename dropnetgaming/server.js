...
async function initDatabase() {
  ...
  if (!Array.isArray(current.duels)) {
    current.duels = [];
    changed = true;
  }
...
function publicState(state) {
  return {
    ...
    duels: Array.isArray(state.duels) ? state.duels.map(publicDuel) : [],
...
function publicDuel(duel) {
  return {
    ...
    players: duel.players || { a: null, b: null },
    winnerUsername: duel.winnerUsername || "",
    winnerNickname: duel.winnerNickname || "",
    eloWin: duel.eloWin || 30,
    eloLose: duel.eloLose || -20,
    log: Array.isArray(duel.log) ? duel.log.slice(-30) : []
  };
}
...
function sanitizeState(input) {
  return {
    ...
    duels: Array.isArray(input.duels) ? input.duels.slice(-50) : [],
...
function duelPlayer(username, nickname) {
  return {
    username,
...
function findDuelPlayerSlot(duel, username) {
  if (duel.players?.a?.username === username) return "a";
  if (duel.players?.b?.username === username) return "b";
  return null;
}

function finishDuel(state, duel, winnerSlot, loserSlot) {
  const winner = duel.players[winnerSlot];
  const loser = duel.players[loserSlot];
  const eloWin = Number(duel.eloWin || 30);
  const eloLose = Number(duel.eloLose || -20);

  duel.status = "finished";
  duel.winnerUsername = winner.username;
  duel.winnerNickname = winner.nickname;
  duel.updatedAt = new Date().toLocaleString("ru-RU");
  duel.finishedAt = duel.updatedAt;
  duel.finishedTs = Date.now();

  const winnerUser = state.users.find((user) => user.username === winner.username);
  const loserUser = state.users.find((user) => user.username === loser.username);

  if (winnerUser) {
    winnerUser.elo = Math.max(100, Number(winnerUser.elo || 1000) + eloWin);
    winnerUser.wins = Number(winnerUser.wins || 0) + 1;
    winnerUser.matches = Number(winnerUser.matches || 0) + 1;
    winnerUser.xp = Number(winnerUser.xp || 0) + 80;
    winnerUser.level = Math.max(1, Math.floor(Number(winnerUser.xp || 0) / 150) + 1);
  }

  if (loserUser) {
    loserUser.elo = Math.max(100, Number(loserUser.elo || 1000) + eloLose);
    loserUser.losses = Number(loserUser.losses || 0) + 1;
    loserUser.matches = Number(loserUser.matches || 0) + 1;
    loserUser.xp = Number(loserUser.xp || 0) + 20;
    loserUser.level = Math.max(1, Math.floor(Number(loserUser.xp || 0) / 150) + 1);
  }

  state.matches.unshift({
    id: Date.now(),
    map: duel.map || "PvP Arena",
    server: "DNG PvP 1v1",
    result: `${winner.nickname} победил ${loser.nickname}`,
    score: "1v1",
    eloChange: eloWin,
    createdAt: duel.finishedAt
  });

  state.tickets.unshift({
    id: Date.now() + 1,
    subject: "PvP дуэль завершена",
    message: `${winner.nickname} победил ${loser.nickname} на ${duel.map || "PvP Arena"}. ELO: +${eloWin} / ${eloLose}`,
    author: "DNG PvP",
    createdAt: duel.finishedAt,
    status: "pvp"
  });
...
app.get("/api/duels", async (_req, res) => {
  try {
    const state = await getState();
    const duels = Array.isArray(state.duels) ? state.duels : [];

    const visible = duels
      .filter((duel) => duel.status !== "finished" || Date.now() - Number(duel.finishedTs || 0) < 1000 * 60 * 60)
      .slice(0, 40)
      .map(publicDuel);

    res.json({ ok: true, duels: visible });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Failed to load duels" });
  }
});
...
app.post("/api/duels", async (req, res) => {
  try {
    const username = String(req.body.username || "").trim();
    const nickname = String(req.body.nickname || username || "").trim();
    const map = String(req.body.map || "Mirage").trim().slice(0, 40);
    ...
    if (!Array.isArray(state.duels)) state.duels = [];

    const activeAlready = state.duels.some((duel) => {
      if (duel.status === "finished") return false;
      return duel.players?.a?.username === username || duel.players?.b?.username === username;
    });

    if (activeAlready) {
      return res.status(409).json({ error: "У тебя уже есть активная PvP-дуэль" });
    }

    const duel = {
      id: "duel_" + Date.now() + "_" + crypto.randomBytes(4).toString("hex"),
      map,
      status: "waiting",
      createdAt: new Date().toLocaleString("ru-RU"),
      updatedAt: new Date().toLocaleString("ru-RU"),
      players: {
        a: duelPlayer(username, nickname),
        b: null
      },
      eloWin: 30,
      eloLose: -20,
      log: [
        {
          ...
          text: `${nickname} создал PvP-дуэль на карте ${map}`
        }
      ]
    };
    ...
    res.json({ ok: true, duel: publicDuel(duel), duels: state.duels.map(publicDuel) });
  } catch (error) {
    console.error(error);
...
app.post("/api/duels/:id/join", async (req, res) => {
  try {
    const username = String(req.body.username || "").trim();
    const nickname = String(req.body.nickname || username || "").trim();
    ...
    const duel = state.duels.find((item) => item.id === req.params.id);

    if (!duel) {
      return res.status(404).json({ error: "Дуэль не найдена" });
    }

    if (duel.status !== "waiting") {
      return res.status(409).json({ error: "Дуэль уже началась" });
    }

    if (duel.players?.a?.username === username) {
      return res.status(409).json({ error: "Нельзя играть против себя" });
    }

    const activeAlready = state.duels.some((item) => {
      if (item.id === duel.id || item.status === "finished") return false;
      return item.players?.a?.username === username || item.players?.b?.username === username;
    });
    ...
    duel.players.b = duelPlayer(username, nickname);
    duel.status = "active";
    duel.updatedAt = new Date().toLocaleString("ru-RU");
    duel.log.push({
      at: duel.updatedAt,
      text: `${nickname} вступил в дуэль. Бой начался.`
    });
...
app.post("/api/duels/:id/action", async (req, res) => {
  try {
    const username = String(req.body.username || "").trim();
    const action = String(req.body.action || "").trim();

    if (!["shoot", "burst", "grenade", "defend"].includes(action)) {
    ...
    if (duel.status !== "active") {
      return res.status(409).json({ error: "Дуэль не активна" });
    }

    const slot = findDuelPlayerSlot(duel, username);

    if (!slot) {
      return res.status(403).json({ error: "Ты не участник этой дуэли" });
    }

    const opponentSlot = slot === "a" ? "b" : "a";
    const player = duel.players[slot];
    const opponent = duel.players[opponentSlot];

    if (!opponent) {
      return res.status(409).json({ error: "Нет соперника" });
    }

    const now = Date.now();
    const cooldown = action === "grenade" ? 2500 : 1000;

    if (now - Number(player.lastActionAt || 0) < cooldown) {
      return res.status(429).json({ error: "Подожди перезарядку" });
    }
    ...
    if (action === "defend") {
      player.shield = true;
      player.hp = Math.min(100, Number(player.hp || 100) + 5);
      text = `${player.nickname} занял укрытие и восстановил 5 HP`;
    } else {
      const configs = {
        shoot: { chance: 0.72, min: 14, max: 24, label: "выстрелил" },
        burst: { chance: 0.55, min: 22, max: 34, label: "дал очередь" },
        grenade: { chance: 0.45, min: 34, max: 48, label: "кинул гранату" }
      };

      const cfg = configs[action];
      const hit = Math.random() <= cfg.chance;

      if (hit) {
        damage = Math.floor(cfg.min + Math.random() * (cfg.max - cfg.min + 1));

        if (opponent.shield) {
          damage = Math.max(5, Math.floor(damage * 0.5));
          opponent.shield = false;
          text = `${player.nickname} ${cfg.label}, но ${opponent.nickname} был в укрытии. Урон: ${damage}`;
        } else {
          text = `${player.nickname} ${cfg.label} по ${opponent.nickname}. Урон: ${damage}`;
        }

        opponent.hp = Math.max(0, Number(opponent.hp || 100) - damage);
      } else {
        text = `${player.nickname} ${cfg.label}, но промахнулся`;
      }
    }
    ...
    duel.log.push(logItem);
    duel.updatedAt = logItem.at;

    if (Number(opponent.hp || 0) <= 0) {
      finishDuel(state, duel, slot, opponentSlot);
    }
    ...
    console.error(error);
    res.status(500).json({ error: "Failed to process duel action" });
  }
});
...
initDatabase()
  .then(() => {
    app.listen(PORT, () => {
      ...
      console.log("Payments, news delete, ELO control, privileges, chat, teams, PvP and Telegram notifications are enabled.");
    });
  })
  .catch((error) => {
